...
public sealed class ReportViewerViewModel : INotifyPropertyChanged
{
    ...
    public int PageCount
    {
        ...
        set
        {
            if (!SetField(ref _pageCount, value))
                return;
    ...
    /// <summary>
    ///     Builds the report again so the viewer reloads it. MAUI Shell drops the native handler when
    ///     switching flyout pages; without a fresh load the viewer comes back with its tiles already
    ///     released and shows a blank page.
    /// </summary>
    public void Reload() => LoadReport();
